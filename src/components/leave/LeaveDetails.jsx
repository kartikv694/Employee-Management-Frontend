...
export default function LeaveDetails() {
    ...
    const changeStatus = async (id,status) => {
        try {
            const {data} = await axios.put(`https://employee-management-backend-ime7.onrender.com/api/v2/employee/leave/status/${id}`,
                {status},
            {
                headers:{
    ...
    const Detail = async ()=>{
        try {
            const {data} = await axios.get(`https://employee-management-backend-ime7.onrender.com/api/v2/employee/leave/details/${_id}`,
                {
                    headers:{
                        Authorization: `Bearer ${auth?.token}`
                    }
