...
export default function EmployeeList() {
    ...
    useEffect(()=>{
        const getEmployees = async ()=>{
            ...
            setLoading(true)
            try {
                const {data} = await axios.get(`https://employee-management-backend-blond.vercel.app/api/v2/employee/all`,{
                    headers:{
                ...
                if (data?.success) 
                {
                    ...
                    const records =  data?.employees.map((emp)=>(
                    {
                        ...
                        profileImage: <img src={`https://employee-management-backend-blond.vercel.app/${emp?.userId?.profileImage}`}
                        width={'100px'}/>,
