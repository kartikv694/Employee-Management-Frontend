...
export default function EditDepartment() {
    ...
    useEffect(()=>{
        const getDepartment = async ()=>{
            try {
                const {data} = await axios.get(`https://employee-management-backend-blond.vercel.app/api/v2/department/get/${_id}`,{
                    headers: {
    ...
    const hanldeSubmit = async (e) => {
        e.preventDefault()
        try {
            const {data} =  await axios.put(`https://employee-management-backend-blond.vercel.app/api/v2/department/update/${_id}`,
            {
