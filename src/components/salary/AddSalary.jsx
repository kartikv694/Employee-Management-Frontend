...
export default function AddSalary() {
    ...
    const handleDepartment = (e) => {
        ...
        const getEmployeeByDept = async () => {
            try {
                const { data } = await axios.get(`http://localhost:8000/api/v2/employee/department/${_id}`, {
                    headers: {
    ...
    const handleChange = async (e) => {
        ...
        if (e.target.name == "employeeId") {
            ...
            const { data } = await axios.get(`http://localhost:8000/api/v2/employee/singlesalary/${empUser?.employeeId || _id}`)
    ...
    const handleSubmit = async (e) => {
        ...
        console.log(empUser)
        try {
            const { data } = await axios.post(`http://localhost:8000/api/v2/employee/salary/add`,
                empUser,
                {
                    headers: {
                        Authorization: `Bearer ${auth?.token}`
